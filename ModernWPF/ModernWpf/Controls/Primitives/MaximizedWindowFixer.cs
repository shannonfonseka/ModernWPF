...
namespace ModernWpf.Controls.Primitives
{
    internal class MaximizedWindowFixer
    {
        ...
        private void OnWindowPosChanging(IntPtr lParam)
        {
            ...
            {
                var windowPosAdjusted = false;

                var placement = NativeMethods.GetWindowPlacement(pos.hwnd);
                if (placement.showCmd == SW.MAXIMIZE)
                {
                    if (GetTaskbarAutoHide(out var edge))
                    {
                        var rect = new MS.Win32.NativeMethods.RECT(pos.x, pos.y, pos.x + pos.cx, pos.y + pos.cy);
                        var monitor = SafeNativeMethods.MonitorFromRect(ref rect, MONITOR_DEFAULTTONEAREST);
                        if (monitor != IntPtr.Zero)
                        {
                            var info = NativeMethods.GetMonitorInfo(monitor);
...
#else
            var transformToDevice = _hwndSource.CompositionTarget.TransformToDevice;
            dpiScaleX = transformToDevice.M11;
            dpiScaleY = transformToDevice.M22;
#endif

            var frameWidth = NativeMethods.GetSystemMetrics(SM.CXSIZEFRAME);
            var frameHeight = NativeMethods.GetSystemMetrics(SM.CYSIZEFRAME);
            var borderPadding = NativeMethods.GetSystemMetrics(SM.CXPADDEDBORDER);
            var borderSize = new Size(frameWidth + borderPadding, frameHeight + borderPadding);
            var borderSizeInDips = DpiHelper.DeviceSizeToLogical(borderSize, dpiScaleX, dpiScaleY);
        ...
        private static bool GetTaskbarAutoHide(out ABEdge edge)
        {
            ...
            if (trayWnd != IntPtr.Zero)
            {
                ...
                var autoHide = Convert.ToBoolean(SHAppBarMessage(ABMsg.ABM_GETSTATE, ref abd));
