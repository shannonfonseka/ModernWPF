...
namespace ModernWpf
{
    public class ThemeResources : ResourceDictionaryEx, ISupportInitialize
    {
        ...
        {
            var targetIndex = DesignMode.DesignModeEnabled ? 1 : 0;
        ...
        {
            var defaultThemeDictionary = ThemeManager.GetDefaultThemeDictionary(key);

            if (ThemeDictionaries.TryGetValue(key, out var themeDictionary))
            {
        ...
        private static bool ContainsDefaultThemeResources(ResourceDictionary dictionary, ResourceDictionary defaultResources)
        {
            ...
            static bool SourceEquals(Uri x, Uri y)
            {
                ...
                var xString = x.IsAbsoluteUri ? x.LocalPath : x.ToString();
                var yString = y.IsAbsoluteUri ? y.LocalPath : y.ToString();
