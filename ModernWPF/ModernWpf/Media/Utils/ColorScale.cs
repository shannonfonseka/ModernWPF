...
namespace ModernWpf.Media.Utils
{
    ...
    internal class ColorScale
    {
        ...
        public Color GetColor(double position, ColorScaleInterpolationMode mode = ColorScaleInterpolationMode.RGB)
        {
            ...
            var scalePosition = (position - _stops[lowerIndex].Position) * (1.0 / (_stops[upperIndex].Position - _stops[lowerIndex].Position));

            switch (mode)
            {
                case ColorScaleInterpolationMode.LAB:
                    var leftLAB = ColorUtils.RGBToLAB(_stops[lowerIndex].Color, false);
                    var rightLAB = ColorUtils.RGBToLAB(_stops[upperIndex].Color, false);
                ...
                case ColorScaleInterpolationMode.XYZ:
                    var leftXYZ = ColorUtils.RGBToXYZ(_stops[lowerIndex].Color, false);
                    var rightXYZ = ColorUtils.RGBToXYZ(_stops[upperIndex].Color, false);
        ...
        public ColorScale Trim(double lowerBound, double upperBound, ColorScaleInterpolationMode mode = ColorScaleInterpolationMode.RGB)
        {
            ...
            var containedStops = new List<ColorScaleStop>(_stops.Length);
