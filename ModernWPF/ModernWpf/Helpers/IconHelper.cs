...
namespace ModernWpf
{
    internal static class IconHelper
    {
        ...
        public static void GetDefaultIconHandles(IntPtr[] largeIconHandle, IntPtr[] smallIconHandle)
        {
            ...
            var iconModuleFile = GetModuleFileName(new HandleRef());
        ...
        {
            var result = NativeMethods.DestroyIcon(icon);
        ...
        private static string GetModuleFileName(HandleRef hModule)
        {
            ...
            while (true)
            {
                var size = NativeMethods.GetModuleFileName(hModule, buffer, buffer.Capacity);
                if (size == 0)
                {
                    throw new Win32Exception();
                }
